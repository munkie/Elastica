...
class Type implements SearchableInterface
{
        ...
        if (($doc->isAutoPopulate()
            ...
            if (!$doc->hasId()) {
                if ($response->hasData('_id')) {
                    $doc->setId($response->getData('_id'));
                }
            }
            if ($response->hasData('_version')) {
                $doc->setVersion($response->getData('_version'));
            }
        }
    ...
    public function getDocument($id, $options = array())
    {
        ...
        if (!$response->hasData('exists') || false == $response->getData('exists')) {
            throw new NotFoundException('doc id ' . $id . ' not found');
        }

        $data = $response->hasData('_source') ? $response->getData('_source') : array();
        $document = new Document($id, $data, $this->getName(), $this->getIndex());
        if ($response->hasData('_version')) {
            $document->setVersion($response->getData('_version'));
        }
    ...
    public function deleteById($id, array $options = array())
    {
        ...
        if ($response->hasData('found') && false == $response->getData('found')) {
